...
void Map::LoadData(string& filename) {
	...
	assert(map_waypoints_x_.size() && "map not loaded, probably path include is missing");

	cout << "Original Map intra point distance min = " << min_dist << "m" << endl;
	cout << "                                  max = " << max_dist << "m" << endl;
	cout << "                                  avg = " << total_dist / (double)line_cnt << "m" << endl;	
	...
	for (size_t i = 1; i < map_waypoints_x_.size(); i++) {
		frenet_s += distance(map_waypoints_x_[i], map_waypoints_y_[i], map_waypoints_x_[i-1], map_waypoints_y_[i-1]);
...
int Map::ClosestWaypoint2(double x, double y)
{
	...
	{
		double dist = distance(x, y, high_res_map_waypoints_x_[i], high_res_map_waypoints_y_[i]);
		if(dist < closestDist)
		{
			closestDist = dist;
			closestWaypoint = i;
...
vector<double> Map::getFrenet(double x, double y, double theta)
{
	...
	double n_x = high_res_map_waypoints_x_[next_wp] - high_res_map_waypoints_x_[prev_wp];
	double n_y = high_res_map_waypoints_y_[next_wp] - high_res_map_waypoints_y_[prev_wp];
	...
	{
		frenet_s += distance(high_res_map_waypoints_x_[i], high_res_map_waypoints_y_[i],
...
void Map::TestClosestWaypoint() 
  {
	...
	for(int i = 0; i < high_res_map_waypoints_x_.size(); i++)
	{
		...
		if (index1 != index2) {
			cout << "Not matching index1=" << index1 << "\t index2=" << index2 << endl;
			double dist1 = distance(high_res_map_waypoints_x_[index1], high_res_map_waypoints_y_[index1], map_x, map_y);
			double dist2 = distance(high_res_map_waypoints_x_[index2], high_res_map_waypoints_y_[index2], map_x, map_y);
