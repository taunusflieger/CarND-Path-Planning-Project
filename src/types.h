...
struct PointCmp {
  ...
  // 2nd derivative 
  double v_ddot;
  PointCmp (double V=0, double V_dot=0, double V_ddot=0) : v(V), v_dot(V_dot), v_ddot(V_ddot) {}
};
...
struct TrajectoryXY {
  XYPoints pts;
  TrajectoryXY(std::vector<double> XPts, std::vector<double> YPts) {pts.xs = XPts; pts.ys = YPts; pts.n = XPts.size();};
  TrajectoryXY() {};
};
...
struct TrajectorySD {
  ...
  TrajectorySD (std::vector<PointCmp> S={}, std::vector<PointCmp> D={}) : path_s(S), path_d(D) {}
};
...
struct PreviousPath {
  TrajectoryXY xy;  
  TrajectorySD sd;    
  PreviousPath (TrajectoryXY XY={}, TrajectorySD SD={}, int N=0) : xy(XY), sd(SD) {};
};
