...
int main(int argc, char **argv) {
  ...
  h.onMessage([&cfg, &map, &just_starting](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
    ...
    // we do the same for x,y path, but for this we get the prev_path from the simulator
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      // delta t between tow calls should be around 20ms. For the first call we set it fix to 20ms
      long dt = (t_msg - t_oldmsg < 0 ? 20 : t_msg - t_oldmsg); // time between two calls (ms)
      auto s = hasData(data);
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          	// j[1] is the data JSON object
      	  	...
      	  	CarLocalizationData vLocal(j[1]["x"],j[1]["y"],j[1]["s"],j[1]["d"],j[1]["yaw"],j[1]["speed"]);
           ...
           /*  
            if (cfg.dbgMain() == 1) {
              ...
              std::cout << "t: " << t_msg << ", dt: " << dt << "ms, x: " << vLocal.x
                        << ", y: " << vLocal.y << ", s: " << vLocal.s << ", v:" << vLocal.speed << std::endl;
            ...
            if (just_starting) {
              ...
              TrajectoryJMT prev_traj = trajectory.generateColdStartPrevPath(egoCar.s, egoCar.d);
              ...
              // egoCar.update_save_states(trajectory.targetState_s, trajectory.targetState_d);
              ...
              //  trajectory.get_jmt_s(), trajectory.get_jmt_d(), cfg.timeIncrement(), num_points);

              // TrajectoryXY traj_xy = trajectory.generateSplineBasedTrajectory(egoCar, XY_points);
              int num_points = cfg.traverseTime()/cfg.timeIncrement(); // TODO: decide if we use this or the number of points defined in the cfg file
              TrajectoryXY traj_xy = trajectory.generateTrajectory(egoCar, BehaviorType::KEEPLANE, prev_path_xy, num_points);
              ...
              prev_path_xy.pts.xs.insert(
                prev_path_xy.pts.xs.end(), NextXY_points.xs.begin(), NextXY_points.xs.end());

              prev_path_xy.pts.ys.insert(
                prev_path_xy.pts.ys.end(), NextXY_points.ys.begin(), NextXY_points.ys.end()); 
            ...
            double t_diff = (((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec))*1000;
